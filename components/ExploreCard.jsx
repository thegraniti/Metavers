...
const ExploreCard = (props) => (
  ...
  className={`relative ${
    props.active === props.id ? 'lg:flex-[3.5] flex-[10]' : 'lg:flex-[0.5] flex-[2]'
  } flex items-center justify-center min-w-[170px] h-[700px] transition-[flex] duration-[0.7s] ease-out-flex cursor-pointer`}
  onClick={() => props.handleClick(props.id)}
  >
    <img src={props.imgUrl} alt={props.title} className="absolute w-full h-full rounded-[24px] object-cover" />
    {props.active !== props.id ? (
      <h3 className='font-semibold sm:text-[26px] text-[18px] text-white absolute z-0 lg:bottom-20 lg:rotate-[-90deg] lg:origin-[0,0]'>{props.title}</h3>
    ) : (
